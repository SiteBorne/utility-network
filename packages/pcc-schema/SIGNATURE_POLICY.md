...
- `policy_hash`: SHA-256 of canonical verification policy document, hex-encoded
  as `sha256:...`
- `canonical()`: JCS canonicalization (RFC 8785) applied to the JSON object
...
Tests use an explicit fixture-only key stored under test fixtures and clearly
marked non-production. Do not generate or commit production private keys.
