...
function main() {
  ...
  for (const serviceSchema of serviceSchemas) {
    ...
    const outputPath = resolve(__dirname, '../../../.temp-combined-schemas', serviceSchema.replace('services/', ''));
  ...
  for (const commonSchema of commonSchemas) {
    ...
    const combined = inlineRefs(original, schemas);
    const outputPath = resolve(__dirname, '../../../.temp-combined-schemas', commonSchema.replace('common/', ''));
    writeFileSync(outputPath, JSON.stringify(combined, null, 2));
  ...
  if (pccSchema) {
    ...
    writeFileSync(resolve(__dirname, '../../../.temp-combined-schemas', 'proof-carrying-context.schema.json'), JSON.stringify(combined, null, 2));
