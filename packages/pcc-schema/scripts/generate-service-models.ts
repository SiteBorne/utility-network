...
function generateTypeScript(schemaPath: string, outputPath: string, additionalSchemas: string[] = []): void {
  ...
  if (schemaPath.includes('/services/') && schemaPath.endsWith('-output.schema.json')) {
    ...
    finalSchemaPath = resolve(ROOT, '.temp-ts-schemas', relativePath.replace(/^(common|services)\//, ''));
  ...
  const args = [
    ...
    '--src-lang', 'schema',
    finalSchemaPath,
    '--just-types', '--no-enums', '--prefer-unions'
  ];
...
function generatePython(schemaPath: string, outputPath: string): void {
  ...
  const relativePath = schemaPath.replace(SCHEMAS_DIR + '/', '');
  const combinedSchemaPath = resolve(ROOT, '.temp-combined-schemas', relativePath.replace(/^(common|services)\//, ''));
  const relativeSchemaPath = combinedSchemaPath.replace(ROOT + '/', '');
  run(VENV_PYTHON, [
    '-m', 'datamodel_code_generator',
    '--input', relativeSchemaPath,
...
function main() {
  ...
  for (const schema of COMMON_SCHEMAS) {
    ...
    generateTypeScript(resolve(SCHEMAS_DIR, 'common', schema), resolve(TS_OUTPUT_DIR, 'common', `${baseName}.ts`), COMMON_SCHEMA_PATHS);
    generatePython(resolve(SCHEMAS_DIR, 'common', schema), resolve(PY_OUTPUT_DIR, 'common', `${baseName}.py`));
  }
  ...
  for (const schema of SERVICE_SCHEMAS) {
    ...
    generateTypeScript(resolve(SCHEMAS_DIR, 'services', schema), resolve(TS_OUTPUT_DIR, 'services', `${baseName}.ts`), additionalSchemas);
    generatePython(resolve(SCHEMAS_DIR, 'services', schema), resolve(PY_OUTPUT_DIR, 'services', `${baseName}.py`), true);
  }
  
  // Also generate PCC schema (already exists but regenerate for consistency)
  console.log('\n--- PCC Schema ---');
  generateTypeScript(resolve(SCHEMAS_DIR, 'proof-carrying-context.schema.json'), resolve(TS_OUTPUT_DIR, 'pcc.ts'), COMMON_SCHEMA_PATHS);
  generatePython(resolve(SCHEMAS_DIR, 'proof-carrying-context.schema.json'), resolve(PY_OUTPUT_DIR, 'pcc.py'));
