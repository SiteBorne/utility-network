/**
 * FIRST-PAID-VERIFY-WORKER-JWT-BUNDLE-INIT-REMEDIATION-01 -- build-output
 * regression gate.
 *
...
const PROBE_ENTRY = path.resolve(__dirname, 'fixtures/cdp-jwt-bundle-probe-entry.ts');
...
function emit(outDir: string, entry?: string): string {
  execFileSync(
    'npx',
    ['wrangler', 'deploy', ...(entry ? [entry] : []), '--dry-run', '--outdir', outDir],
    {
      cwd: REPO_ROOT,
      stdio: 'pipe',
  ...
  return path.join(outDir, `${entry ? path.basename(entry, '.ts') : 'index'}.js`);
}
...
describe('emitted Worker bundle: CDP JWT initialisation', () => {
  beforeAll(() => {
    workDir = mkdtempSync(path.join(tmpdir(), 'cdp-jwt-bundle-init-'));
