 ...
 * dedicated `NEVERMINED_RECONCILE_VERIFY_V2=1` mode performs GET-only discovery
 * and can never reach registration. The separate
 * `NEVERMINED_REGISTER_VERIFY_V2=1` mode uses the builder credential only,
...
const SERVICE_ID = 'verify_agent_output.v2' as const;
const RECONCILE_ONLY = process.env.NEVERMINED_RECONCILE_VERIFY_V2 === '1';
const REGISTER = process.env.NEVERMINED_REGISTER_VERIFY_V2 === '1';

describe.skipIf(!RECONCILE_ONLY && !REGISTER)(
  'SUN-1000 checkpoint 1O-B — v2 verify agent output registration only',
  () => {
