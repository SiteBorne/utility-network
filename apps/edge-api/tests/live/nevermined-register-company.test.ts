/**
 * SUN-0900B checkpoint 2J — registration-only operator entry point for
 * `company_evidence_graph.v1`.
 *
 * The harness is guarded independently from every payment/live probe. A
 * dedicated `NEVERMINED_RECONCILE_COMPANY=1` mode performs GET-only discovery
 * and can never reach registration. The separate
 * `NEVERMINED_REGISTER_COMPANY=1` mode uses the builder credential only,
...
const RECONCILE_ONLY = process.env.NEVERMINED_RECONCILE_COMPANY === '1';
const REGISTER = process.env.NEVERMINED_REGISTER_COMPANY === '1';

describe.skipIf(!RECONCILE_ONLY && !REGISTER)(
  'SUN-0900B checkpoint 2J — company evidence registration only',
  () => {
