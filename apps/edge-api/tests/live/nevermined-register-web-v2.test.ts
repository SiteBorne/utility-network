 ...
 * dedicated `NEVERMINED_RECONCILE_WEB_V2=1` mode performs GET-only discovery
 * and can never reach registration. The separate
 * `NEVERMINED_REGISTER_WEB_V2=1` mode uses the builder credential only,
...
const SERVICE_ID = 'web_context_verified.v2' as const;
const RECONCILE_ONLY = process.env.NEVERMINED_RECONCILE_WEB_V2 === '1';
const REGISTER = process.env.NEVERMINED_REGISTER_WEB_V2 === '1';

describe.skipIf(!RECONCILE_ONLY && !REGISTER)(
  'SUN-1000 checkpoint 1O-B — v2 web context registration only',
  () => {
    it('reconciles first, registers at most once after proven absence, and validates GET read-back', async () => {
      ...
      if (RECONCILE_ONLY === REGISTER) {
        throw new Error('select exactly one web reconciliation/registration mode');
      }
      ...
      expect(declaration.plan.gross_buyer_amount_atomic).toBe('9000');
      ...
      console.log('Web registration initial reconciliation (sanitized):', {
      ...
      if (reconciliation.state === 'existing') {
        ...
        if (exact.state !== 'EXACT_EXISTING') {
          throw new Error(
            `web registration identity matched but authoritative read-back conflicted (${exact.state === 'CONFLICT' ? exact.reason : exact.state})`
          );
        }
        ...
        if (RECONCILE_ONLY) {
          // eslint-disable-next-line no-console
          console.log('Web registration is authoritatively NO_MATCH; read-only mode stopping.');
          return;
        }
        ...
        // eslint-disable-next-line no-console
        console.log('Web registration submitted (sanitized):', {
        ...
        if (reconciliation.state !== 'existing') {
          throw new Error(
            `web post-registration reconciliation was ${reconciliation.state}; outcome is ambiguous and must not be retried`
          );
        }
        ...
        throw new Error(
          `web registration reconciliation was ${reconciliation.state}; refusing mutation`
        );
      }
      ...
      // eslint-disable-next-line no-console
      console.log('Web registration read-back (sanitized):', {
        registration_mutations: registrationMutations,
        agent_id: agentId,
