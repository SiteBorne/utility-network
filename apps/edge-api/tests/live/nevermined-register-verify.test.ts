/**
 * SUN-0900B checkpoint 2K — registration-only operator entry point for
 * `verify_agent_output.v1`.
 *
 * `NEVERMINED_RECONCILE_VERIFY=1` permits GET-only discovery. The separate
 * `NEVERMINED_REGISTER_VERIFY=1` mode uses only the builder credential,
 * reconciles before mutation, permits at most one `registerAgentAndPlan` call
 * after proven absence, and requires authoritative GET read-back validation.
 * It never creates subscriber/payment state or calls a service runtime.
 */
...
const RECONCILE_ONLY = process.env.NEVERMINED_RECONCILE_VERIFY === '1';
const REGISTER = process.env.NEVERMINED_REGISTER_VERIFY === '1';

describe.skipIf(!RECONCILE_ONLY && !REGISTER)(
  'SUN-0900B checkpoint 2K — verify agent output registration only',
  () => {
    it('reconciles first, registers at most once after proven absence, and validates GET read-back', async () => {
      ...
      if (process.env.NVM_SUBSCRIBER_API_KEY) {
        throw new Error('subscriber credential must be removed from verify registration process');
      }
      ...
      if (RECONCILE_ONLY === REGISTER) {
        throw new Error('select exactly one verify reconciliation/registration mode');
      }
      ...
      expect(declaration.plan.gross_buyer_amount_atomic).toBe('19000');
      ...
      console.log('Verify registration initial reconciliation (sanitized):', {
      ...
      if (reconciliation.state === 'existing') {
        ...
        if (exact.state !== 'EXACT_EXISTING') {
          throw new Error(
            `verify registration identity matched but authoritative read-back conflicted (${exact.state === 'CONFLICT' ? exact.reason : exact.state})`
          );
        }
        ...
        if (RECONCILE_ONLY) {
          // eslint-disable-next-line no-console
          console.log('Verify registration is authoritatively NO_MATCH; read-only mode stopping.');
          return;
        }
        ...
        // eslint-disable-next-line no-console
        console.log('Verify registration submitted (sanitized):', {
        ...
        if (reconciliation.state !== 'existing') {
          throw new Error(
            `verify post-registration reconciliation was ${reconciliation.state}; outcome is ambiguous and must not be retried`
          );
        }
        ...
        throw new Error(
          `verify registration reconciliation was ${reconciliation.state}; refusing mutation`
        );
      }
      ...
      // eslint-disable-next-line no-console
      console.log('Verify registration read-back (sanitized):', {
        registration_mutations: registrationMutations,
        agent_id: agentId,
