...
describe('storage-alert-receiver-entrypoint — SUN-1222C delivery containment (STORAGE_ALERT_DELIVERY_ENABLED)', () => {
  it('DELIVERY_ENABLED absent, ordinary request -> no SMTP', async () => {
    const env = makeEnv();
    const res = await worker.fetch(post(`alert/${TOKEN}`, VALID_PAYLOAD), env);
  ...
  it('THE RACE THIS CONTAINS: r2_delete_failures > 0, DELIVERY_ENABLED absent, scheduled-shaped caller -> receiver reached (existing behavior), SMTP transport NOT reached', async () => {
    // Exactly what `reclaimStaleArtifactsScheduled` sends once a real R2
    // delete failure occurs -- no qualification header or bypass exists any
    // longer (removed once the one supervised, mailbox-confirmed
    // qualification attempt this containment was built to gate was
    // complete): this flag is now the sole determinant of a real send.
    const env = makeEnv();
